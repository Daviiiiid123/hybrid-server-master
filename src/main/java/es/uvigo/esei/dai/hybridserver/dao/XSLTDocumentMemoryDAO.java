...
/**
 * Implementación en memoria del DAO para documentos XSLT.
 * Utiliza un ConcurrentHashMap para garantizar thread-safety.
 * Mantiene también un mapa de asociaciones UUID -> XSD UUID.
 */
public class XSLTDocumentMemoryDAO implements XSLTDocumentDAO {
    
    private final Map<String, String> documents;
    private final Map<String, String> xsdAssociations;
    
    public XSLTDocumentMemoryDAO() {
        this.documents = new ConcurrentHashMap<>();
        this.xsdAssociations = new ConcurrentHashMap<>();
    }
    
    public XSLTDocumentMemoryDAO(Map<String, String> initialDocuments) {
        this.documents = new ConcurrentHashMap<>();
        this.xsdAssociations = new ConcurrentHashMap<>();
    ...
    @Override
    public boolean saveDocument(String uuid, String content, String xsdUuid) {
        if (uuid == null || content == null || xsdUuid == null) {
            return false;
        }
        documents.put(uuid, content);
        xsdAssociations.put(uuid, xsdUuid);
        return true;
    }
    ...
    public String getAssociatedXSD(String uuid) {
        return xsdAssociations.get(uuid);
    }
}
